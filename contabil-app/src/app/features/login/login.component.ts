...
@Component({
      ...
      <form class="login-card" (ngSubmit)="entrar()">
        ...
        <label class="field">
          ...
          <input pInputText type="email" name="email" [(ngModel)]="email" autocomplete="username"
                 placeholder="contador@erpkit.local" required />
        </label>
  ...
  styles: [`
    ...
    .brand i { font-size: 1.9rem; color: var(--color-copper); }
  ...
  entrar(): void {
    ...
    this.auth.login(this.email, this.senha).pipe(finalize(() => this.loading.set(false))).subscribe({
      ...
      error: (err) => this.msg.add({
        ...
        detail: err?.status === 401 ? 'E-mail ou senha inválidos.' : 'Não foi possível conectar à contabilidade.',
      }),
    });
  }
}
